...
export default async function DashboardPage() {
  try {
    ...
    // データベース接続をチェック（エラーが発生してもリダイレクトしない）
    ...
    if (!isConnected) {
      ...
      // データベース接続に失敗した場合は、エラー状態を表示する
      return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
          <Navigation />
          <div className="container mx-auto px-4 py-8">
            <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
              <h1 className="text-2xl font-bold text-red-800 mb-4">データベース接続エラー</h1>
              <p className="text-red-700 mb-4">
                データベースに接続できませんでした。しばらく時間をおいてから再度お試しください。
              </p>
              <button 
    ...
    // ユーザーのカードセットを取得（エラーハンドリング付き）
    let myCardSets: CardSet[] = [];
    try {
      myCardSets = await prisma.cardSet.findMany({
    ...
    let dailyUsage = null;
    try {
      dailyUsage = await prisma.dailyUsage.findUnique({
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {myCardSets.map((cardSet) => (
                  <div key={cardSet.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                    ...
                    <p className="text-sm text-gray-600 mb-3 line-clamp-2">{cardSet.description || '説明なし'}</p>
    ...
    // エラーが発生した場合は、エラー状態を表示する（リダイレクトしない）
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
        <Navigation />
        <div className="container mx-auto px-4 py-8">
          <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
            ...
            <p className="text-red-700 mb-4">
              ダッシュボードの読み込み中にエラーが発生しました。しばらく時間をおいてから再度お試しください。
            </p>
